...
### ✉️ Email (Gmail API, OAuth2)

`EmailService` sends all transactional email (account-opening OTP, password reset,
new-account details, online-banking-activation) through the **Gmail API**,
authenticated with an OAuth2 refresh token — **not SMTP, and not a username/password
or Gmail App Password.**

| Variable | Purpose | Required |
|---|---|---|
| `GMAIL_CLIENT_ID` | OAuth2 Client ID (Google Cloud Console → APIs & Services → Credentials) | Yes |
| `GMAIL_CLIENT_SECRET` | OAuth2 Client Secret for the same client | Yes |
| `GMAIL_REFRESH_TOKEN` | Long-lived refresh token, obtained once via Google's OAuth consent flow, scoped to `https://www.googleapis.com/auth/gmail.send` only | Yes |
| `GMAIL_SENDER_EMAIL` | The Gmail mailbox these credentials belong to — used as both the Gmail API user and the `From:` address | Yes |
| `GMAIL_SENDER_NAME` | Display name on the `From:` header | No — defaults to `DKS Bank` |

**None of these have a properties-file fallback, on purpose.** Every environment
(local dev, Render, CI) must set all four required variables explicitly. If any are
missing, email sending fails gracefully with a clear message — it does not crash
the application, and no credential is ever written to a file or logged.

To obtain `GMAIL_CLIENT_ID` / `GMAIL_CLIENT_SECRET` / `GMAIL_REFRESH_TOKEN`:
1. In Google Cloud Console, create (or reuse) an OAuth 2.0 Client ID of type "Desktop app" and enable the Gmail API for that project.
2. Run Google's standard OAuth2 installed-app consent flow once, requesting only the `gmail.send` scope, using the account that should send bank emails.
3. Store the resulting client ID, client secret, and refresh token as environment variables (or Render's secret environment variable store) — never in a file inside this repository.

**Migrated from SMTP:** this project previously used `SMTP_HOST` / `SMTP_PORT` /
`SMTP_USERNAME` / `SMTP_PASSWORD` / `SMTP_FROM_EMAIL` / `SMTP_FROM_NAME` with a Gmail
App Password. Those variables and `src/main/resources/email.properties` are no
longer read by the application — see the note in that file's directory for cleanup.
